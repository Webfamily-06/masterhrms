  ...
  // Clear any existing chart DOM nodes to prevent React StrictMode duplicate overlapping
  Array.from(document.querySelectorAll('div[id]')).forEach(el => {
    if (el.id.includes('chart') || el.id.includes('spark') || el.id.includes('pie') || el.id.includes('gauge') || el.id.includes('radar') || el.id === 's-line' || el.id === 's-col' || el.id === 's-bar') {
      el.innerHTML = '';
      if (el.parentElement && el.parentElement.style.position === 'relative' && el.parentElement.style.overflow === 'hidden') {
