  ...
  return (
    <PlanGuard moduleName="CRM & Sales Pipeline" requiredPlan="starter">
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-5">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">
              Track leads, manage deals, and monitor your full sales funnel.
            </p>
          </div>
        ...
        {/* Sneat Pro CRM & Sales Pipeline KPI Widgets */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            { title: "Won Revenue", value: formatSystemAmount(metrics.won, sysConfig?.currency), desc: "Successfully closed sales", icon: CheckCircle2, color: "text-[oklch(0.60_0.17_155)] bg-[oklch(0.60_0.17_155/0.10)]" },
            { title: "Active Deals", value: `${metrics.active} In Progress`, desc: "Leads under active negotiation", icon: TrendingUp, color: "text-[oklch(0.73_0.16_75)] bg-[oklch(0.73_0.16_75/0.10)]" },
            { title: "Win Rate", value: `${metrics.winRate}%`, desc: "Funnel conversion efficiency", icon: BarChart3, color: "text-[oklch(0.60_0.20_200)] bg-[oklch(0.60_0.20_200/0.10)]" },
          ].map((m) => (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 overflow-x-auto">
            {STAGES.map((stage) => {
              ...
              return (
                <div
                  key={stage.id}
                  className={`rounded-2xl border p-3 space-y-2 min-w-[160px] ${stage.bg}`}
                >
                  <div
                    className={`text-xs font-extrabold ${stage.color} flex items-center justify-between`}
                  >
                  ...
                  <div className="space-y-2">
                    {stageLeads.length === 0 ? (
                      ...
                      stageLeads.map((lead) => (
                        <Card
                          key={lead.id}
                          className="p-2.5 space-y-1.5 shadow-xs cursor-pointer hover:border-primary/50 transition-colors"
          ...
          <DialogContent className="sm:max-w-[520px]">
            ...
            <div className="grid grid-cols-2 gap-3 py-2 text-xs">
              ...
              <div className="space-y-1">
                ...
                <Select
                  value={form.stage}
                  onValueChange={(v) => setForm({ ...form, stage: v as CrmLead["stage"] })}
                >
          ...
          <DialogContent className="sm:max-w-[440px]">
              ...
              <div className="space-y-4 py-2 text-xs">
                <div className="flex items-center gap-3 p-3 rounded-xl bg-secondary/40 border">
                  ...
                  <div>
                    ...
                    <div className="text-muted-foreground flex items-center gap-1">
                      <Building2 className="size-3" /> {viewingLead.company}
                    </div>
                    <Badge className="mt-1 text-[10px]">
                      {STAGES.find((s) => s.id === viewingLead.stage)?.label}
                    </Badge>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  {[
                    {
                      label: "Deal Value",
                      value: formatSystemAmount(viewingLead.value, sysConfig?.currency),
                      icon: DollarSign,
                    },
                ...
                {viewingLead.nextFollowUp && (
                  <div className="p-2.5 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-800 dark:text-amber-300 flex items-center gap-2 text-xs">
                    ...
                    <span className="font-bold">Next Follow-up: {viewingLead.nextFollowUp}</span>
                  </div>
                )}
                {viewingLead.notes && (
            ...
            <DialogFooter>
              <Button
                variant="outline"
                onClick={() => {
                  if (viewingLead) openEdit(viewingLead);
                  setViewingLead(null);
                }}
              >
