  ...
  const { data: importHistory = [] } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        if (page?.content && Array.isArray(page.content)) return page.content as ImportRecord[];
        ...
        <div className="flex items-center justify-between border-b pb-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">
              Import CSV/XML ledger data from Tally Prime with field mapping and validation.
            </p>
          </div>
        ...
        <Tabs defaultValue="import">
          <TabsList>
            <TabsTrigger value="import" className="text-xs gap-1.5">
              <Upload className="size-3.5" /> Import Wizard
            </TabsTrigger>
            <TabsTrigger value="history" className="text-xs gap-1.5">
              <History className="size-3.5" /> Import History ({importHistory.length})
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="import" className="mt-4">
              ...
              {(
                ...
                return (
                  <div key={s.id} className="flex items-center gap-2">
                    <div
                      className={`flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-full border transition-colors ${isCurrent ? "bg-primary text-primary-foreground border-primary" : isDone ? "bg-emerald-100 text-emerald-700 border-emerald-300" : "bg-secondary text-muted-foreground border-border"}`}
                    >
            ...
            {step === "upload" && (
              <Card
                className="border-2 border-dashed border-orange-500/40 bg-orange-500/5 rounded-2xl p-12 text-center cursor-pointer hover:border-orange-500 transition-colors"
              ...
              >
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Supports CSV, XML, or XLSX format
                </p>
                <Button
                  className="mt-4 bg-orange-600 hover:bg-orange-700 text-white font-bold gap-2"
                  onClick={(e) => {
                    e.stopPropagation();
              ...
              <div className="space-y-4">
                ...
                <Card className="p-4 space-y-3">
                  <div className="flex items-center gap-2 text-sm font-bold border-b pb-2">
                    <Map className="size-4 text-primary" /> Map Tally Fields → Master ERP Fields
                  </div>
                  <div className="space-y-2">
                    {TALLY_FIELDS.map((tallyField) => (
                      <div key={tallyField} className="flex items-center gap-3 text-xs">
                        <div className="flex-1 p-2 rounded-lg bg-orange-500/10 border border-orange-500/20 font-mono font-bold text-orange-700 dark:text-orange-300">
                          {tallyField}
                        </div>
                        <ArrowRight className="size-4 text-muted-foreground shrink-0" />
                        <Select
                          value={fieldMap[tallyField] || ""}
                          onValueChange={(v) => setFieldMap({ ...fieldMap, [tallyField]: v })}
                        >
                ...
                <div className="flex gap-3">
                  <Button variant="outline" onClick={resetWizard} className="text-xs">
                    ← Start Over
                  </Button>
            ...
            {step === "preview" && (
              <div className="space-y-4">
                ...
                <div className="overflow-x-auto rounded-xl border">
                  <table className="w-full text-xs">
                    <thead className="bg-secondary/50 text-muted-foreground">
                      <tr>
                        {["Ledger", "Date", "Type", "Debit", "Credit", "Narration"].map((h) => (
                ...
                <div className="flex gap-3">
                  <Button variant="outline" onClick={() => setStep("map")} className="text-xs">
                    ← Back to Mapping
                  </Button>
                  <Button
                    onClick={handleImport}
                    disabled={isImporting}
                    className="font-bold gap-2 text-xs bg-orange-600 hover:bg-orange-700 text-white"
                  >
                    {isImporting ? (
                    ...
                    {isImporting
                      ? `Importing ${uploadedFile?.rows} records...`
                      : `Import All ${uploadedFile?.rows} Records`}
                  </Button>
                </div>
              </div>
            )}
              ...
              <div className="py-12 text-center space-y-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    {importHistory[0]?.importedRows} ledger entries imported from "
                    {importHistory[0]?.fileName}"
                    {importHistory[0]?.errors > 0
                      ? ` (${importHistory[0].errors} errors skipped)`
                      : " with no errors"}
                    .
                  </p>
                </div>
            ...
            {importHistory.length === 0 ? (
              ...
              <div className="overflow-x-auto rounded-xl border">
                <table className="w-full text-xs">
                  <thead className="bg-secondary/50 text-muted-foreground">
                    <tr>
                      {["File", "Total", "Imported", "Errors", "Status", "Imported At"].map((h) => (
                        <th key={h} className="p-2.5 text-left font-semibold">
                          {h}
                        </th>
                      ))}
