...
const DEFAULT_TENANT_CATEGORIES = [
  "Branding",
  "Contracts & Legal",
  "Receipts & Bills",
  "ID Proofs",
...
const DEFAULT_SEED_ASSETS: TenantMediaAsset[] = [
  {
    id: "med-1",
    name: "Enterprise Brand Vector.png",
  ...
  {
    id: "med-2",
    name: "Vendor Master Contract NDA.pdf",
...
function TenantMediaPage() {
  ...
  const [uploadFolderChoice, setUploadFolderChoice] = useState("Company Logos");
  const [uploadCategoryChoice, setUploadCategoryChoice] = useState("Logos");
  ...
  const { data: mediaData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      if (!tenantId)
        return {
          folders: DEFAULT_TENANT_FOLDERS,
          categories: DEFAULT_TENANT_CATEGORIES,
          files: DEFAULT_SEED_ASSETS,
        };
      try {
        const page = await api.get(`/cms/pages/${slugKey}`);
        if (page?.content && typeof page.content === "object" && "files" in page.content) {
          return {
            folders: (page.content.folders ?? DEFAULT_TENANT_FOLDERS) as string[],
            categories: (page.content.categories ?? DEFAULT_TENANT_CATEGORIES) as string[],
            files: (page.content.files ?? DEFAULT_SEED_ASSETS) as TenantMediaAsset[],
          };
        }
        // Seed default
        const defaultState = {
        ...
        await api.put(`/cms/pages/${slugKey}`, {
          title: `Media Library ${tenantId}`,
  ...
  async function persistMediaState(updatedData: {
    folders?: string[];
    categories?: string[];
    files?: TenantMediaAsset[];
  }) {
    if (!tenantId) return;
    const payload = {
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b pb-5">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Button
              ...
              className="gap-1.5 text-xs bg-primary font-bold"
            >
          ...
          <div className="space-y-4">
            ...
            {isLoading ? (
              ...
              <Card className="p-12 text-center text-xs text-muted-foreground italic space-y-2">
                ...
                <p>
                  No assets found matching folder "{selectedFolder}" and category "
                  {selectedCategory}".
                </p>
              </Card>
            ) : (
              <div className="grid sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
                {filteredAssets.map((ast) => (
                  <Card
                    key={ast.id}
                    className="overflow-hidden group hover:border-primary/50 transition-all flex flex-col justify-between"
                  >
