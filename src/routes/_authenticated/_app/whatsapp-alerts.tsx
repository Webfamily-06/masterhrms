...
function WhatsAppAlertsPage() {
  ...
  const [botForm, setBotForm] = useState<BotRule>({
    id: "",
    keyword: "",
    replyText: "",
    action: "reply_text",
    ...
    queryFn: async () => {
      try {
        ...
        return {
          ...
          botRules: DEFAULT_BOT_RULES,
          logs: [] as WaDeliveryLog[],
          config: {},
        };
      } catch {
        return {
  ...
  function saveTemplate() {
    ...
    if (editingTemplate) {
      updatedTemplates = templates.map((t) =>
        t.id === editingTemplate.id ? { ...editingTemplate, ...templateForm } : t,
      );
  ...
  function saveBotRule() {
    if (!botForm.keyword.trim() || !botForm.replyText.trim())
      return toast.error("Keyword and reply text are required");
      ...
      <div className="space-y-5">
        <div className="flex items-center justify-between border-b pb-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">
              Automated outbound alerts & interactive inbound WhatsApp bot auto-responder.
            </p>
          </div>
        ...
        <Tabs defaultValue="bot">
          ...
          <TabsContent value="bot" className="mt-4 space-y-5">
            ...
            <Card className="p-4 bg-emerald-500/10 border-emerald-500/30 space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Set this Webhook URL in your Twilio / Meta WhatsApp Business API Dashboard to
              ...
              <Card className="p-4 border space-y-3 flex flex-col justify-between h-[450px]">
                <CardHeader className="p-0 pb-2 border-b">
                  ...
                  <CardDescription className="text-[10px]">
                    Test inbound keyword auto-replies live.
                  </CardDescription>
                </CardHeader>
                ...
                <div className="flex-1 overflow-y-auto space-y-2 p-2 rounded-xl bg-slate-900 text-white text-xs font-sans">
                  {simChatHistory.map((chat, idx) => (
                    ...
                    >
                      <div
                        className={`p-2.5 rounded-xl max-w-[85%] ${chat.sender === "user" ? "bg-emerald-600 text-white rounded-br-none" : "bg-slate-800 text-slate-100 rounded-bl-none border border-slate-700"}`}
                      >
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    className="h-8 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs shrink-0"
                  >
                    Send
                  </Button>
                </div>
          ...
          <TabsContent value="templates" className="mt-4 space-y-4">
            <div className="flex justify-between items-center">
              <p className="text-xs text-muted-foreground">
                Message templates with trigger rules and variable placeholders.
              </p>
              <Button
                size="sm"
                onClick={() => {
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        <Button
                          ...
                          onClick={() => {
                            ...
                            setTemplateForm({ name: t.name, body: t.body, trigger: t.trigger });
                            setIsTemplateModalOpen(true);
                          }}
                        >
          ...
          <TabsContent value="config" className="mt-4 max-w-xl space-y-5">
            <Card className="p-5 space-y-4 border">
              <CardHeader className="p-0 pb-3 border-b">
                <CardTitle className="text-sm font-bold flex items-center gap-2">
                  <Settings2 className="size-4 text-primary" /> WhatsApp Business API Credentials
                </CardTitle>
                <CardDescription className="text-xs">
                  Configure your Twilio or WhatsApp Cloud API credentials for message delivery.
                </CardDescription>
              </CardHeader>
              <div className="space-y-3 text-xs">
                <div className="space-y-1">
                  <Label className="text-xs font-semibold">API Key / Access Token</Label>
                  <Input
                    type="password"
                    value={apiKey}
                ...
                <div className="space-y-1">
                  <Label className="text-xs font-semibold">Sender Phone Number ID</Label>
                  <Input
                    value={apiPhone}
                    onChange={(e) => setApiPhone(e.target.value)}
                ...
                <Button
                  onClick={() => {
                    persist.mutate({ templates, botRules, logs, config: { apiKey, apiPhone } });
            ...
            <Card className="p-5 space-y-3 border">
              <CardHeader className="p-0 pb-2">
                <CardTitle className="text-sm font-bold flex items-center gap-2">
                  <Send className="size-4 text-emerald-600" /> Send Test Message
                </CardTitle>
              </CardHeader>
              <div className="flex gap-2">
                <Input
                ...
                <Button
                  ...
                  className="font-bold text-xs shrink-0 bg-emerald-600 hover:bg-emerald-700 text-white gap-1.5"
                >
                  {isSendingTest ? (
          ...
          <DialogContent className="sm:max-w-[440px]">
            ...
            <div className="space-y-3 py-2 text-xs">
              <div className="space-y-1">
                ...
                <Input
                  value={botForm.keyword}
                  onChange={(e) => setBotForm({ ...botForm, keyword: e.target.value })}
                  placeholder="e.g. PAYSLIP, LEAVE, SALARY"
              ...
              <div className="space-y-1">
                ...
                <Textarea
                  rows={4}
                  value={botForm.replyText}
                  onChange={(e) => setBotForm({ ...botForm, replyText: e.target.value })}
                  placeholder="Enter automated WhatsApp response text..."
                  className="text-xs font-mono"
                />
              </div>
            </div>
            <DialogFooter>
              <Button variant="outline" onClick={() => setIsBotModalOpen(false)}>
                Cancel
              </Button>
              <Button
                onClick={saveBotRule}
