        ...
        <div className="flex items-center justify-between border-b pb-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">
              Configure Razorpay, process test payments, and view transaction logs.
            </p>
          </div>
          ...
          {[
            {
              ...
              value: formatSystemAmount(metrics.total, sysConfig?.currency),
              icon: DollarSign,
        ...
        <Tabs defaultValue="config">
          <TabsList>
            <TabsTrigger value="config" className="text-xs gap-1.5">
              <Settings2 className="size-3.5" /> API Configuration
            </TabsTrigger>
            <TabsTrigger value="test" className="text-xs gap-1.5">
              <Play className="size-3.5" /> Test Payment
            </TabsTrigger>
            <TabsTrigger value="transactions" className="text-xs gap-1.5">
              <History className="size-3.5" /> Transactions ({transactions.length})
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="config" className="mt-4 max-w-xl space-y-4">
            <Card className="p-5 space-y-4">
              <CardHeader className="p-0 pb-3 border-b">
                <CardTitle className="text-sm font-bold flex items-center gap-2">
                  <Shield className="size-4 text-primary" /> Razorpay API Credentials
                </CardTitle>
                <CardDescription className="text-xs">
                  Your credentials are stored securely and never exposed publicly.
                </CardDescription>
              </CardHeader>
              <div className="space-y-3 text-xs">
                <div className="space-y-1">
                  ...
                  <p className="text-[10px] text-muted-foreground">
                    Live or Test key from Razorpay Dashboard → Settings → API Keys
                  </p>
                </div>
                <div className="space-y-1">
            ...
            <Card className="p-5 space-y-3">
              <CardHeader className="p-0 pb-3 border-b">
                <CardTitle className="text-sm font-bold flex items-center gap-2">
                  <Webhook className="size-4 text-primary" /> Webhook Configuration
                </CardTitle>
                <CardDescription className="text-xs">
                  Add this secret in Razorpay Dashboard → Webhooks for payment status updates.
                </CardDescription>
              </CardHeader>
          ...
          <TabsContent value="test" className="mt-4 max-w-md">
            <Card className="p-5 space-y-4">
              <CardHeader className="p-0 pb-3 border-b">
                <CardTitle className="text-sm font-bold flex items-center gap-2">
                  <Play className="size-4 text-emerald-600" /> Test Payment Flow
                </CardTitle>
                <CardDescription className="text-xs">
                  Simulate a payment to verify your Razorpay integration is working.
                </CardDescription>
              </CardHeader>
              <div className="space-y-3 text-xs">
                <div className="space-y-1">
                ...
                <Button
                  ...
                  disabled={isTesting}
                  className="w-full font-bold gap-2 h-11 bg-emerald-600 hover:bg-emerald-700 text-white"
                >
                  {isTesting ? (
                    <Loader2 className="size-4 animate-spin" />
                  ) : (
                ...
                {transactions.length > 0 && transactions[0]?.paymentId && (
                  <div className="p-3 rounded-xl border bg-secondary/30 space-y-1 text-[11px] font-mono">
                    ...
                    <div className="text-muted-foreground">
                      Payment ID:{" "}
                      <span className="text-foreground font-bold">{transactions[0].paymentId}</span>
                    </div>
                    <div className="text-muted-foreground">
                      Amount:{" "}
                      <span className="text-foreground font-bold">₹{transactions[0].amount}</span>
                    </div>
                    <div className="text-muted-foreground">
                      Status:{" "}
                      <Badge
                        className={`text-[10px] ml-1 ${transactions[0].status === "captured" ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700"}`}
                      >
                    ...
                    {transactions.map((t) => (
                      <tr key={t.id} className="border-t hover:bg-secondary/20">
                        ...
                        <td className="p-2.5 font-extrabold">
                          {formatSystemAmount(t.amount, sysConfig?.currency)}
                        </td>
                        <td className="p-2.5">
