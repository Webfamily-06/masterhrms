...
const DEFAULT_MARKETPLACE_ADDONS: Addon[] = [
  {
    id: "a1",
    ...
    docs_url: "#",
    version: "v2.4.0",
    features: ["Automated payslip PDF delivery", "Shift clock-in/out alert triggers", "Two-factor OTP delivery"],
    screenshots: [],
  },
  {
    id: "a2",
    ...
    docs_url: "#",
    version: "v3.1.0",
    features: ["TCP/IP direct socket sync", "Automated shift assignment", "Offline punch buffering"],
    screenshots: [],
  },
  {
    id: "a3",
    ...
    docs_url: "#",
    version: "v2.0.0",
    features: ["20+ Multi-LLM API Integration (OpenAI, Gemini, Claude, Groq)", "Multi-Language Generation (English, Tamil, Hindi, Spanish, French)", "Tone & Creativity Controls", "1-Click Copy & Markdown Export"],
    screenshots: [],
  },
  {
    id: "a4",
    ...
    docs_url: "#",
    version: "v1.8.0",
    features: ["GSTIN validation", "Multi-page PDF parsing", "Auto ledger posting"],
    screenshots: [],
  },
  {
    id: "a5",
    ...
    docs_url: "#",
    version: "v1.0.0",
    features: ["Cascading OKR alignment tree", "Key Result check-ins & blocker notes", "Multi-rater 360° appraisals", "Quarterly review cycles"],
    screenshots: [],
  },
  {
    id: "a6",
    ...
    docs_url: "#",
    version: "v1.0.0",
    features: ["Device registry & barcodes", "1-click employee handover & returns", "Maintenance logs & repair costs", "Automated warranty alerts"],
    screenshots: [],
  },
  {
    id: "a7",
    ...
    docs_url: "#",
    version: "v2.0.0",
    features: ["17 Chart of Accounts", "Balanced journal postings", "Live Balance Sheet & P&L", "Budget vs Actual analytics"],
    screenshots: [],
  },
  {
    id: "a8",
    ...
    tagline: "Drag-and-drop form creator with custom fields and response exports",
    ...
    docs_url: "#",
    version: "v1.5.0",
    features: ["Drag-and-drop visual builder", "Conditional field logic", "Export submissions to CSV/Excel"],
    screenshots: [],
  },
  {
    id: "a9",
    ...
    tagline: "2-way automated voucher & ledger sync for accounting software",
    ...
    docs_url: "#",
    version: "v4.0.1",
    features: ["Auto voucher creation", "Ledger sync", "Tax invoice mapping"],
    screenshots: [],
  },
  {
    id: "a10",
    ...
    docs_url: "#",
    version: "v2.1.0",
    features: ["UPI auto-collect", "QR generation", "Instant payment verification"],
    screenshots: [],
  },
  {
    id: "a11",
    ...
    tagline: "Sync calendars, Gmail invitations, and Drive document backups",
...
function MarketplaceAdmin() {
  ...
  const {
    data: addons = DEFAULT_MARKETPLACE_ADDONS,
    isLoading,
    refetch,
  } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        return DEFAULT_MARKETPLACE_ADDONS;
      } catch {
        return DEFAULT_MARKETPLACE_ADDONS;
      }
    },
  ...
  const upsert = useMutation({
    mutationFn: async (a: Partial<Addon> & { id?: string }) => {
      const currentList = [...addons];
      if (a.id) {
        const idx = currentList.findIndex((item) => item.id === a.id);
        if (idx >= 0) currentList[idx] = { ...currentList[idx], ...a } as Addon;
      } else {
        const newRecord = { ...a, id: `addon-${Date.now()}` } as Addon;
        currentList.push(newRecord);
      }

      await api.put("/cms/pages/system-addons-catalog", {
        title: "System Addons Catalog",
        meta_description: "500+ ecosystem addons catalogue",
        content: currentList,
        published: true,
      });
    },
  ...
  const del = useMutation({
    mutationFn: async (id: string) => {
      const currentList = addons.filter((item) => item.id !== id);
      await api.put("/cms/pages/system-addons-catalog", {
        title: "System Addons Catalog",
        meta_description: "500+ ecosystem addons catalogue",
        content: currentList,
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="p-4 shadow-xs border space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Select value={statusFilter} onValueChange={setStatusFilter}>
              ...
              <SelectContent>
                ...
                <SelectItem value="beta" className="text-xs">
                  Status: Beta
                </SelectItem>
      ...
      <div className="flex items-center justify-between text-xs text-muted-foreground px-1">
        <span>
          Showing {filteredAddons.length} of {addons?.length ?? 0} addons
        </span>
        {categoryFilter !== "All" && (
      ...
      {isLoading ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {filteredAddons.map((a) => (
            <Card
              key={a.id}
              className="group hover:border-primary/50 transition-all flex flex-col justify-between shadow-xs"
            >
              <CardContent className="p-5 space-y-4">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex flex-wrap gap-1 justify-end">
                    ...
                    <Badge
                      variant={a.status === "available" ? "secondary" : "outline"}
              ...
              <div className="px-5 pb-4 pt-0 flex items-center justify-end gap-2 border-t mt-auto pt-3">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="text-xs text-muted-foreground hover:text-primary mr-auto flex items-center gap-1"
                >
            ...
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2">
                ...
                {"id" in editing && editing.id
                  ? `Edit Addon — ${editing.name}`
                  : "Create New Addon"}
              </DialogTitle>
              <DialogDescription>
                Configure addon details, PNG icon images, monthly pricing, features, and
                screenshots.
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="grid md:grid-cols-2 gap-4 py-2">
              ...
              <Field label="Monthly Price ($ USD)">
                <Input
                  ...
                  onChange={(e) =>
                    setEditing({ ...editing, price_monthly: parseFloat(e.target.value) || 0 })
                  }
                  placeholder="0.00"
                />
              </Field>
              ...
              <div className="md:col-span-2 p-4 rounded-xl border bg-secondary/10 space-y-3">
                <Label className="text-xs font-bold flex items-center justify-between">
                  <span className="flex items-center gap-1.5">
                    <ImageIcon className="size-4 text-primary" /> PNG Icon Image (Upload file or
                    URL)
                  </span>
                  {editing.icon && (
                ...
                <div className="flex flex-col sm:flex-row items-center gap-3">
                  ...
                  <div className="size-16 shrink-0 rounded-xl border bg-background grid place-items-center overflow-hidden p-1 shadow-xs">
                    {editing.icon &&
                    (editing.icon.startsWith("http") || editing.icon.startsWith("data:")) ? (
                      <img
                        src={editing.icon}
                        alt="Preview"
              ...
              <div className="md:col-span-2 p-4 rounded-xl border bg-secondary/10 space-y-3">
                <div className="flex items-center justify-between">
                  <Label className="text-xs font-bold flex items-center gap-1.5">
                    <ImageIcon className="size-4 text-primary" /> PNG Screenshots Gallery (
                    {screenshots.length})
                  </Label>
                ...
                {screenshots.length > 0 ? (
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 pt-1">
                    {screenshots.map((src, idx) => (
                      <div
                        key={idx}
                        className="relative group rounded-lg border bg-background overflow-hidden aspect-video"
                      >
                        <img
                          src={src}
