...
function NotificationTemplatesAdmin() {
  ...
  // 1. REALTIME QUERY: Fetch notification triggers from MySQL API
  const {
    data: notifications,
    isLoading,
    refetch,
    ...
    queryFn: async () => {
      try {
        const page = await api.get("/cms/pages/system-notification-templates");
        if (page?.content && Array.isArray(page.content.notifications)) {
          return page.content.notifications as NotificationTemplate[];
        }
  ...
  // 2. REALTIME MUTATION: Save triggers to MySQL API
  const saveMutation = useMutation({
    mutationFn: async (updatedList: NotificationTemplate[]) => {
      await api.put("/cms/pages/system-notification-templates", {
        title: "System Notification Templates",
        meta_description: "Realtime notification triggers and push templates",
        content: { notifications: updatedList },
        published: true,
      });
    },
    onSuccess: () => {
      toast.success("Notification triggers synced with MySQL");
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b pb-5">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Configure system notification triggers for In-App, Push Notifications, SMS, and Email
            events.
          </p>
        </div>
