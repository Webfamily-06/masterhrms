...
function GoogleAnalyticsAdmin() {
  ...
  // 1. REALTIME QUERY: Fetch GA config from MySQL API
  const {
    data: analyticsData,
    isLoading,
    refetch,
    ...
    queryFn: async () => {
      try {
        const page = await api.get("/cms/pages/system-analytics-config");
        if (page?.content) {
          return page.content as AnalyticsConfig;
        }
  ...
  // 2. REALTIME MUTATION: Save GA config to MySQL API
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success("Google Analytics tracking configuration saved to MySQL!");
          ...
          <Card className="p-6 border shadow-xs space-y-4 max-w-xl">
            <div className="space-y-1.5">
              ...
              <p className="text-[11px] text-muted-foreground">
                Find this ID in Google Analytics Admin &gt; Data Streams.
              </p>
            </div>
          </Card>
