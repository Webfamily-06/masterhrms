...
function EmailTemplatesAdmin() {
  ...
  const [selectedId, setSelectedId] = useState("et-1");
  const [activeView, setActiveView] = useState<"code" | "preview">("preview");

  // 1. REALTIME QUERY: Fetch email templates from MySQL API
  const {
    data: templates,
    isLoading,
    refetch,
    ...
    queryFn: async () => {
      try {
        ...
        if (page?.content && Array.isArray(page.content.templates)) {
          return page.content.templates as EmailTemplate[];
        }
  ...
  // 2. REALTIME MUTATION: Save updated templates to MySQL API
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b pb-5">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Design HTML transactional email templates with live HTML preview and real-time MySQL
            sync.
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="grid lg:grid-cols-[280px_1fr] gap-6 items-start">
          ...
          <Card className="p-2 border shadow-xs">
            <div className="p-3 text-xs font-bold text-muted-foreground uppercase border-b mb-2">
              System Templates
            </div>
            <div className="space-y-1">
              {list.map((t) => (
                ...
                >
                  ...
                  <div
                    className={`text-[10px] ${current.id === t.id ? "text-primary-foreground/80" : "text-muted-foreground"}`}
                  >
                    {t.category}
                  </div>
                </button>
