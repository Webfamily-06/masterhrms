...
function DedicatedBlogsManager() {
  ...
  // 1. REALTIME QUERY: Fetch custom blog categories
  const { data: blogCategories = DEFAULT_BLOG_CATEGORIES } = useQuery({
    queryKey: ["realtime-blog-categories"],
    queryFn: async () => {
      try {
        const page = await api.get("/cms/pages/system-blog-categories");
  ...
  // 2. REALTIME QUERY: Fetch all Blogs from MySQL API
        ...
        if (Array.isArray(list)) {
          return list.filter(
            (p: any) =>
              ...
              p.slug === "resource-blogs" ||
              p.content?.category === "Blogs" ||
              p.content?.category === "ERP Systems",
          ) as CmsPage[];
        }
        return [];
      } catch {
  ...
  async function persistCategories(updated: string[]) {
    await api.put("/cms/pages/system-blog-categories", {
  ...
  function handleOpenEdit(blog: CmsPage) {
    ...
    setForm({
      ...
      readTime: blog.content?.readTime || "5 min read",
      coverImage:
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      const cleanSlug = (form.slug || form.title.toLowerCase().replace(/[^a-z0-9-]/g, "-")).replace(
        /^blog-/,
        "",
      );
    ...
    onSuccess: () => {
      toast.success(
        editingBlog ? "Blog article updated in real-time!" : "New Blog created & published to CMS!",
      );
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b pb-5">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Create blog articles, manage custom blog categories, upload thumbnails, and publish to
            CMS.
          </p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-card p-4 rounded-2xl border shadow-xs">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto overflow-x-auto">
          <Select value={selectedCat} onValueChange={setSelectedCat}>
            ...
            <SelectContent>
              <SelectItem value="All Categories" className="text-xs">
                All Categories
              </SelectItem>
                ...
                <div>
                  ...
                  <div className="h-44 relative bg-secondary/40 overflow-hidden">
                    ...
                    <div className="absolute top-3 left-3">
                      <Badge
                        variant="outline"
                        className="text-[10px] font-mono bg-background/80 backdrop-blur-md"
                      >
                  ...
                  <CardHeader className="p-4 pb-2 space-y-1">
                    <div className="flex items-center gap-2 text-[10px] text-muted-foreground font-mono">
                      <span className="flex items-center gap-1">
                        <User className="size-3 text-primary" /> {b.content?.author || "Master ERP"}
                      </span>
                      <span>·</span>
                      <span>
                        {b.updated_at ? new Date(b.updated_at).toLocaleDateString() : "Published"}
                      </span>
                    </div>
          ...
          <div className="space-y-4 py-2 text-xs">
            ...
            <div className="space-y-1">
              <Label className="text-xs font-semibold">
                Full Blog Article Content (Markdown / HTML)
              </Label>
              <Textarea
            ...
            <div className="flex items-center justify-between p-3 rounded-xl border bg-secondary/30 pt-2">
              <div>
                ...
                <p className="text-[11px] text-muted-foreground">
                  Make this blog visible on public /resources and landing pages
                </p>
              </div>
              <Switch
          ...
          <div className="space-y-4 py-2 text-xs">
            ...
            <div className="space-y-1.5 border rounded-xl p-3 bg-secondary/20 max-h-56 overflow-y-auto">
              <div className="text-[10px] uppercase font-bold text-muted-foreground mb-1">
                Active Categories ({blogCategories.length})
              </div>
