...
function DedicatedCaseStudiesManager() {
  ...
  // 1. REALTIME QUERY: Fetch custom categories
  const { data: industriesList = DEFAULT_INDUSTRIES } = useQuery({
    queryKey: ["realtime-case-study-categories"],
    queryFn: async () => {
      try {
        const page = await api.get("/cms/pages/system-case-study-categories");
        if (page?.content && Array.isArray(page.content.categories)) {
          return page.content.categories as string[];
        }
  ...
  // 2. REALTIME QUERY: Fetch all Case Studies from MySQL API
        ...
        if (Array.isArray(list)) {
          return list.filter(
            (p: any) =>
              ...
              p.slug === "resource-case-studies" ||
              p.content?.category === "Case Studies",
          ) as CmsPage[];
        }
        return [];
      } catch {
  ...
  async function persistCategories(updated: string[]) {
    await api.put("/cms/pages/system-case-study-categories", {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      const cleanSlug = (
        form.slug || form.clientCompany.toLowerCase().replace(/[^a-z0-9-]/g, "-")
      ).replace(/^case-study-/, "");
      ...
      if (editingCaseStudy) {
        await api.put(`/cms/pages/${finalSlug}`, {
          title: form.title,
          slug: finalSlug,
    ...
    onSuccess: () => {
      toast.success(
        editingCaseStudy
          ? "Case Study updated in real-time!"
          : "New Case Study published to public site!",
      );
  ...
  async function handleDeleteCaseStudy(slug: string, title: string) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b pb-5">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Create customer success stories, manage industry categories, upload thumbnails, and
            publish ROI metrics.
          </p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-card p-4 rounded-2xl border shadow-xs">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto overflow-x-auto">
          <Select value={selectedIndustry} onValueChange={setSelectedIndustry}>
            ...
            <SelectContent>
              <SelectItem value="All Industries" className="text-xs">
                All Industries
              </SelectItem>
      ...
      {isLoading ? (
        ...
        <Card className="p-12 text-center text-xs text-muted-foreground italic space-y-2">
          ...
          <p>
            No case studies found. Click "Create New Case Study" to add customer ROI success
            stories.
          </p>
        </Card>
      ) : (
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredCaseStudies.map((cs) => {
            ...
            return (
              <Card
                key={cs.id}
                className="flex flex-col justify-between overflow-hidden hover:border-primary/60 transition-all group"
              >
                <div>
                  ...
                  <CardHeader className="p-4 pb-2 space-y-1.5">
                    ...
                    <div className="p-2.5 rounded-xl border bg-emerald-500/10 border-emerald-500/20 text-emerald-700 dark:text-emerald-300 font-semibold text-xs flex items-center gap-2">
                      ...
                      <span className="truncate">
                        {cs.content?.roiResult ||
                          cs.meta_description ||
                          "Transformed enterprise ops"}
                      </span>
                    </div>
                  </CardHeader>
                </div>
                  ...
                  <div className="pt-3 border-t flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <Button
                        ...
                        onClick={() => handleDeleteCaseStudy(cs.slug, cs.title)}
                        title="Delete"
                      >
        ...
        <DialogContent className="sm:max-w-[620px] max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              {editingCaseStudy ? "Edit Customer Case Study" : "Create New Case Study"}
            </DialogTitle>
            <DialogDescription className="text-xs">
              Publish customer success story, record ROI metrics, and upload plant/office thumbnail
              images.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-2 text-xs">
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-center">
                  <span className="bg-secondary px-2.5 py-1.5 rounded-l-md border border-r-0 font-mono text-[10px] text-muted-foreground">
                    case-study-
                  </span>
                  <Input
            ...
            <div className="space-y-1">
              <Label className="text-xs font-semibold">
                Full Case Study Solution & Results (Markdown / HTML)
              </Label>
              <Textarea
            ...
            <div className="flex items-center justify-between p-3 rounded-xl border bg-secondary/30 pt-2">
              <div>
                ...
                <p className="text-[11px] text-muted-foreground">
                  Make this case study visible on public /resources and customer pages
                </p>
              </div>
              <Switch
          ...
          <div className="space-y-4 py-2 text-xs">
            ...
            <div className="flex gap-2">
              ...
              <Button
                ...
                className="bg-primary font-bold text-xs shrink-0 gap-1"
              >
            ...
            <div className="space-y-1.5 border rounded-xl p-3 bg-secondary/20 max-h-56 overflow-y-auto">
              <div className="text-[10px] uppercase font-bold text-muted-foreground mb-1">
                Active Industry Sectors ({industriesList.length})
              </div>
