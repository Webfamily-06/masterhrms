  ...
  head: () => ({
    meta: [
      { title: "Open Graph Metadata Previewer — Master HRMS Tools" },
      {
        name: "description",
        content:
          "Test and inspect Open Graph meta tags (og:title, og:description, og:image) for any website URL.",
      },
    ],
  }),
});
...
function OgPreviewPage() {
  ...
  // Validate URL format
  function validateAndNormalizeUrl(urlStr: string): {
    valid: boolean;
  ...
  async function fetchOgData(targetUrl: string) {
    ...
    try {
      ...
      if (microRes.ok) {
        ...
        if (json.status === "success" && json.data) {
          ...
          if (!title && !description && !image) {
            ...
            setErrorMessage(
              "No Open Graph data found on this page. The webpage does not contain og:title, og:description, or og:image tags.",
            );
            setLoading(false);
            return;
          }
      ...
      setErrorMessage(
        "Failed to fetch webpage. The site may be down, unreachable, or blocking CORS proxy requests.",
      );
    } finally {
      setLoading(false);
    }
            ...
            <Card className="p-12 text-center shadow-xs border">
              <div className="flex flex-col items-center gap-3">
                ...
                <p className="text-xs text-muted-foreground max-w-sm">
                  Connecting to URL and extracting <code>og:title</code>,{" "}
                  <code>og:description</code>, and <code>og:image</code>.
                </p>
              </div>
            </Card>
          )}
            ...
            <Card className="border-destructive/30 bg-destructive/5 shadow-xs">
              <CardContent className="p-6 flex items-start gap-4">
                ...
                {errorType === "fetch_failed" && (
                  <Link2 className="size-6 text-destructive shrink-0 mt-0.5" />
                )}
                {errorType === "no_og_data" && (
                  <FileQuestion className="size-6 text-amber-600 shrink-0 mt-0.5" />
                )}
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <Badge variant="secondary" className="gap-1 px-3 py-1 text-xs">
                  <CheckCircle2 className="size-3.5 text-emerald-600" /> Open Graph Metadata Parsed
                  Successfully
                </Badge>
                    ...
                    <p className="text-xs text-muted-foreground leading-relaxed line-clamp-3">
                      {ogData.description || (
                        <span className="italic opacity-70">
                          No og:description meta tag found on this webpage.
                        </span>
                      )}
                    </p>
                  </CardContent>
                ...
                <Card className="shadow-md border">
                  <CardHeader className="p-4 border-b">
                    ...
                    <CardDescription className="text-xs">
                      Extracted meta tags used by social platforms (LinkedIn, Twitter, Facebook,
                      Slack).
                    </CardDescription>
                  </CardHeader>
