...
const DEFAULT_ADDONS_DICT: Record<string, AddonRow> = {
  "whatsapp-alerts": {
    id: "a1",
    ...
    queryFn: async () => {
      try {
        ...
        if (Array.isArray(page?.content)) {
          const found = page.content.find((a: any) => a.slug === slug);
          if (found) return found as AddonRow;
        }
        return DEFAULT_ADDONS_DICT[slug] || null;
      } catch {
        return DEFAULT_ADDONS_DICT[slug] || null;
      }
    },
          ...
          <div className="flex gap-2 flex-wrap justify-center items-center">
            ...
            <Badge variant="secondary" className="font-mono">
              {addon.price_monthly === 0
                ? "Free"
                : `₹${addon.price_monthly > 100 ? addon.price_monthly : addon.price_monthly * 80}/mo`}
            </Badge>
          </div>
        </div>
      </PageHero>
