...
(function () {
	...
	// Sidebar - highlight the menu item matching the current page URL, and
	// scroll it into view, overriding any hardcoded "active" classes left in
	// the markup so only the current page's item stays active. Runs first,
	// before any plugin-dependent code below that could throw on pages
	// missing a given plugin (which would otherwise abort this whole script
	// and skip the highlighting).
	(function setActiveMenuFromUrl() {
		var currentPage = location.pathname.split('/').pop();
		if (!currentPage) currentPage = 'index.html';

		var links = document.querySelectorAll('.sidebar-menu a[href], .sidebar-right ul a[href]');

		links.forEach(function (link) {
			link.classList.remove('active', 'subdrop');
		});
		document.querySelectorAll('.sidebar-menu li.active, .sidebar-right ul li.active').forEach(function (li) {
			li.classList.remove('active');
		});

		// This template ships several duplicate sidebar markup blocks per page
		// (default, horizontal, two-column, stacked, mini layouts) and only one
		// is visible at a time via CSS - so several links can match currentPage.
		// Keep all of them for scrolling purposes and pick the visible one below.
		var matchedLinks = [];
		...
		links.forEach(function (link) {
			...
			// Mark ancestor submenu toggles active/expanded too.
			var parentUl = link.closest('ul');
			while (parentUl) {
				var parentLi = parentUl.parentElement ? parentUl.parentElement.closest('li') : null;
				if (!parentLi) break;
		...
		if (matchedLinks.length) {
			// Wait for layout (sidebar-inner/simplebar wrapping) to settle before scrolling.
			setTimeout(function () {
				var visibleLink = matchedLinks.find(function (link) {
					return link.offsetWidth > 0 || link.offsetHeight > 0;
				});
				if (!visibleLink) return;

				var scrollContainer = visibleLink.closest('.simplebar-content-wrapper')
					|| visibleLink.closest('.sidebar-scroll')
					|| visibleLink.closest('[data-simplebar]');
				if (!scrollContainer) return;

				var containerRect = scrollContainer.getBoundingClientRect();
				var linkRect = visibleLink.getBoundingClientRect();
				var offset = (linkRect.top - containerRect.top) + scrollContainer.scrollTop
					- (scrollContainer.clientHeight / 2) + (linkRect.height / 2);
